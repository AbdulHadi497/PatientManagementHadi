...
class Database:
    def __init__(self):
        self.dbConnection = sqlite3.connect("hospital.db")
        self.dbCursor = self.dbConnection.cursor()
        self.dbCursor.execute(
            "CREATE TABLE IF NOT EXISTS patient_table (id PRIMARYKEY text, firstname text, lastname text, dateOfBirth "
            "text, monthOfBirth text, yearOfBirth text, gender text, address text, contactNumber text, emailAddress "
            "text, bloodType text, history text, doctor text)")

    def __del__(self):
        self.dbCursor.close()
        self.dbConnection.close()

    def insert(self, ID, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber,
               emailAddress, bloodType, history, doctor):
        self.dbCursor.execute("INSERT INTO patient_table VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (
            ID, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber,
            emailAddress, bloodType, history, doctor))
    ...
    def update(self, firstname, lastname, dateOfBirth, monthOfBirth, yearOfBirth, gender, address, contactNumber,
        ...
        self.dbCursor.execute(
            "UPDATE patient_table SET firstname = ?, lastname = ?, dateOfBirth = ?, monthOfBirth = ?, yearOfBirth = ?,"
            "gender = ?, address = ?, contactNumber = ?, emailAddress = ?, bloodType = ?,"
            "history = ?, doctor = ? WHERE id = ?",
...
def validate(id, firstname, lastname, contactNumber, emailAddress):
...
class InsertWindow:
    ...
    def insert(self):
        ...
        self.test = validate(self.idEntry.get(), self.firstnameEntry.get(), self.lastnameEntry.get(),
...
class HomePage:
    def __init__(self):
        ...
        self.homePageWindow.wm_title("Hospital Management")
        bg_color = "Green"
        fg_color = "white"
        tkinter.Label(self.homePageWindow, relief=tkinter.GROOVE, fg=fg_color, bg=bg_color, text="Home Page",
                      font=("times new roman", 20, "bold"), width=85).grid(pady=20, column=1, row=1)
